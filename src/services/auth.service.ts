...
export async function login(payload: LoginPayload): Promise<AuthTokens> {
  const { data } = await api.post<AuthTokens>("/v1/auth/login/", payload);
  return data;
}

export async function register(payload: RegisterPayload): Promise<User> {
  const { data } = await api.post<User>("/v1/auth/register/", payload);
  return data;
}
...
export async function getMe(fetcher?: <T>(path: string) => Promise<T>): Promise<User | null> {
  try {
    if (fetcher) {
      // Server-side
      return await fetcher<User>("/v1/users/me/");
    }
    // Client-side
