  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`px-2.5 py-1 rounded-sm border text-[11px] font-black transition-all duration-150 focus:outline-none flex items-center gap-2 group ${btnCls} h-8 ${compact ? "min-w-0" : "min-w-36"}`}
      >
...
export function TransactionsToolbar({
  ...
  const activeCount = [typeFilter, dateFilter, search].filter(Boolean).length;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex sm:hidden items-center gap-2">
          ...
          {filtersOpen && (
            <div className="absolute left-0 mt-3 z-50 w-72 bg-white border border-gray-200 rounded-sm shadow-2xl p-4 flex flex-col gap-5 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest border-b border-slate-50 pb-2">
                Showing {totalResults} Transactions
              </div>
              
              <div className="space-y-2">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><Activity size={10} /> Type</span>
                <div className="flex flex-col gap-1 pr-1">
                  {typeOptions.map((opt) => (
                    <button key={opt.key} onClick={() => { setTypeFilter(opt.key); setFiltersOpen(false); }}
                      className={`w-full text-left px-3 py-2 text-[10px] font-black tracking-widest uppercase transition-colors rounded-sm ${typeFilter === opt.key ? "bg-orange-500 text-white" : "text-gray-500 hover:bg-slate-50"}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><Clock size={10} /> Time</span>
                <div className="flex flex-col gap-1 pr-1">
                  {dateOptions.map((opt) => (
                    <button key={opt.key} onClick={() => { setDateFilter(opt.key); setFiltersOpen(false); }}
                      className={`w-full text-left px-3 py-2 text-[10px] font-black tracking-widest uppercase transition-colors rounded-sm ${dateFilter === opt.key ? "bg-orange-500 text-white" : "text-gray-500 hover:bg-slate-50"}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
        ...
        <SearchBar search={search} setSearch={setSearch} placeholder="Search tx id, product..." />
      </div>

      {/* ── TABLET (sm → lg) ── */}
      <div className="hidden sm:flex lg:hidden items-center gap-2">
        <SearchBar search={search} setSearch={setSearch} />
        <DropdownFilter label="Type" value={typeFilter} onChange={setTypeFilter} options={typeOptions} icon={Activity} compact />
        <DropdownFilter label="Time" value={dateFilter} onChange={setDateFilter} options={dateOptions} icon={Clock} compact />
        {isFiltered && (
      ...
      <div className="hidden lg:flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 px-3 h-8 bg-slate-50 border border-slate-100 rounded-sm">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{totalResults} Transactions</span>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <SearchBar search={search} setSearch={setSearch} placeholder="Search tx id, user, product..." />
        ...
        <div className="hidden sm:flex flex-wrap items-center gap-2 py-0.5">
          ...
          {typeFilter && (
            <span className="flex items-center gap-1.5 px-2 py-1 bg-orange-50 text-orange-600 text-[10px] font-bold border border-orange-100 rounded-sm">
              <Activity size={9} className="text-orange-400" />
              {typeFilter}
              <button onClick={() => setTypeFilter("")} className="ml-0.5 text-orange-400 hover:text-red-500 transition-colors cursor-pointer"><X size={11} strokeWidth={3} /></button>
            </span>
          )}
          {dateFilter && (
            <span className="flex items-center gap-1.5 px-2 py-1 bg-blue-50 text-blue-600 text-[10px] font-bold border border-blue-100 rounded-sm">
              ...
              {dateOptions.find(o => o.key === dateFilter)?.label || dateFilter}
              <button onClick={() => setDateFilter("")} className="ml-0.5 text-blue-400 hover:text-red-500 transition-colors cursor-pointer"><X size={11} strokeWidth={3} /></button>
            </span>
          )}
        </div>
      )}
