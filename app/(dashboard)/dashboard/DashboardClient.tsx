...
function RangeTabs({
  ...
  function handleCustomClick() {
    ...
    if (customBtnRef.current) {
      ...
      // vertical: flip above if not enough room below (dropdown ~180px tall)
      if (spaceBelow < 200) {
      ...
      // horizontal: align right edge of dropdown to right edge of button, clamp to viewport
  ...
  function handleOtherTabClick(v: RangeLabel) {
    setDropdownOpen(false);
    onChange(v);
  }
    ...
    <div className="flex items-center bg-slate-100/80 rounded-sm p-0.5 gap-0.5 w-fit">
      {RANGE_TABS.map((tab) => {
        ...
        return (
          <button
            key={tab.value}
            type="button"
            onClick={() => handleOtherTabClick(tab.value)}
            className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-sm transition-all duration-150 whitespace-nowrap cursor-pointer ${
              value === tab.value
                ? "bg-orange-500 text-white shadow-sm"
                : "text-slate-400 hover:text-slate-600 hover:bg-white/50 active:text-slate-700"
      ...
      {dropdownOpen && (
        <div
          ref={dropdownRef}
          style={pos}
          className="bg-white border border-slate-200 rounded-sm shadow-lg p-4 flex flex-col gap-3 w-64 animate-in fade-in slide-in-from-top-1 duration-150"
        >
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="h-8 px-2.5 text-[11px] font-bold border border-slate-200 rounded-sm bg-white text-slate-700 focus:outline-none focus:border-orange-400 cursor-pointer hover:border-slate-300 transition-colors w-full"
            />
          </div>
          <div className="flex flex-col gap-1">
          ...
          {customStart && customEnd && (
            <button
              ...
              className="h-7 text-[10px] font-black uppercase tracking-widest bg-orange-500 text-white rounded-sm hover:bg-orange-600 transition-colors"
            >
              Apply
            </button>
          )}
...
export default function DashboardClient({ initialStats, initialRange = "month" }: Readonly<DashboardClientProps>) {
...
const chartData = useMemo(() => {
    ...
    (transactions?.recent_activity ?? []).forEach(tx => {
      ...
      if (!groups[isoKey]) groups[isoKey] = { date: dateObj.toLocaleDateString(), isoKey, receive: 0, sale: 0 };
  ...
  return (
    <div className="px-4 py-5 md:px-6 md:py-6 space-y-6 max-w-350 mx-auto">
      ...
      {/* ── KPI GRID: 1 col on Mobile, 3 on Tablet+ ── */}
      <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4 transition-opacity duration-200 ${loading ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
        {/* Products */}
        <Link href="/products" className="bg-white border border-slate-200 rounded-sm p-4 flex flex-col gap-3 hover:border-orange-300 transition-colors group h-full">
          ...
          <div className="flex flex-col gap-1.5 mt-auto pt-2 border-t border-slate-100">
             <div className="flex items-center justify-between">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">All items</span>
        ...
        <Link href="/inventory" className="bg-white border border-slate-200 rounded-sm p-4 flex flex-col gap-3 hover:border-orange-300 transition-colors group h-full">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Low Stock</p>
          ...
          <div className="flex flex-col gap-1.5 mt-auto pt-2 border-t border-slate-100">
             <div className="flex items-center justify-between">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Out of stock</span>
        ...
        <Link href="/transactions" className="bg-white border border-slate-200 rounded-sm p-4 flex flex-col gap-3 hover:border-orange-300 transition-colors group h-full">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Transactions</p>
              <p className="text-2xl md:text-3xl font-black text-slate-900 leading-none tabular-nums tracking-tighter mt-1 group-hover:text-orange-600 transition-colors">
          ...
          <div className="flex flex-col gap-1.5 mt-auto pt-2 border-t border-slate-100">
             <div className="flex items-center justify-between">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">This period</span>
