...
export default function ProductsPage() {
  ...
  // Debounce: update debouncedSearch 200ms after typing stops
  useEffect(() => {
  ...
  const displayed = useMemo(() => {
    ...
    if (sortKey) {
      list.sort((a, b) => {
        const av = sortKey === "cost_per_unit" ? Number.parseFloat(a.cost_per_unit) : a[sortKey];
        const bv = sortKey === "cost_per_unit" ? Number.parseFloat(b.cost_per_unit) : b[sortKey];
  ...
  // ── Table content (extracted to avoid nested ternaries) ──────────────────
  ...
  // ── Save button label (extracted to avoid nested ternary) ────────────────
  let saveLabel = editing ? "Save Changes" : "Add Product";
