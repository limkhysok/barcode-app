...
export default function InventoryClient({ initialRecords, initialProducts }: Readonly<{
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-2xl font-black text-gray-900 leading-none tabular-nums">{stats.totalQty.toLocaleString()}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-2xl font-black text-gray-900 leading-none tabular-nums">{stats.sites}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-2xl font-black text-gray-900 leading-none tabular-nums">{stats.needsReorder}</p>
      ...
      {stats.total > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4">
          ...
          <div className="flex h-2 rounded-full overflow-hidden gap-0.5">
            {stats.healthy > 0 && (
              <div className="bg-green-500 rounded-full transition-all" style={{ width: `${(stats.healthy / stats.total) * 100}%` }} />
            )}
            {stats.moderate > 0 && (
              <div className="bg-amber-400 rounded-full transition-all" style={{ width: `${(stats.moderate / stats.total) * 100}%` }} />
            )}
            {stats.low > 0 && (
              <div className="bg-red-500 rounded-full transition-all" style={{ width: `${(stats.low / stats.total) * 100}%` }} />
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-7 space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <CustomSelect id="product" label="Product"
                value={form.product || ""} placeholder="Select a product…"
                onChange={(v) => setForm((f) => ({ ...f, product: Number.parseInt(v) }))}
                options={products.map((p) => ({ value: p.id, label: `${p.product_name} (${p.category})` }))} />

              <div className="grid grid-cols-2 gap-4">
                <CustomSelect id="site" label="Site" value={form.site} placeholder="Select a site…"
